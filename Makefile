...
TARGET := chatd
SRC := src/chatd.c src/client.c src/handlers.c src/protocol.c
INTEG_TEST_SUPPORT_SRC := src/client.c src/handlers.c src/protocol.c
OBJ := $(SRC:.c=.o)
...
$(INTEG_TEST_BIN): $(INTEG_TEST_SRC)
	$(CC) $(CFLAGS) -o $@ $< $(INTEG_TEST_SUPPORT_SRC)
